...
box::use(bt_utils = bamtools/utils)
tool <- "bamtools"
dat$data <- dat$data |>
...
d1 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "summary", tb = "tbl1")
d2 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "covx", tbl = "tbl2")
...
d1 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "coverage")
d1$res |>
...
d1 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "exoncvg")
p1 <- d1$res |>
...
d1 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "genes")
d2 <- dat$data |>
  tw_utils$unnest_data_multi(tbl_nm, schemas_all = dat$schemas, subnest = "cvg")
```
